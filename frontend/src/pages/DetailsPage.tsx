...
export default function DetailsPage() {
  ...
  const { movieDetails, error, loading, loadMovieDetails } =
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <Heart active={has(movieId)} onClick={() => toggle(movieId)} size="lg" />
      </div>

      <div className="text-sm text-gray-600">
        <span>⭐ {movieDetails.voteAverage ?? "—"}</span>
        {movieDetails.runtime ? (
          <span className="mx-2">• {movieDetails.runtime} min</span>
        ) : null}
        {movieDetails.releaseDate ? (
          <span className="mx-2">• {movieDetails.releaseDate}</span>
        ) : null}
      </div>
